...
typedef uintptr_t palQueueID_t;
...
/*! \brief Create and initialize an queue.
*
* @param[in] queueLen  Length of the queue.
* @param[in] itemSize  Size of the item of the queue.
* @param[out] queueID   The ID handle for the created queue. In case of error, this value is `NULL`.
*
* \return PAL_SUCCESS when the queue was created successfully.
* \return PAL_ERR_NO_MEMORY when there is no memory resource available to create an queue object.
*/
palStatus_t pal_QueueCreate(const unsigned long queueLen, const unsigned long itemSize, palQueueID_t* queueID);

/*! \brief Create and initialize an queue with static buffer.
*
* @param[in] queueLen  Length of the queue.
* @param[in] itemSize  Size of the item of the queue.
* @param[in] buffer    pointer to the buffer which can hold the maximum number of items that can be in the queue at any one time.
* @param[out] queueID   The ID handle for the created queue. In case of error, this value is `NULL`.
*
* \return PAL_SUCCESS when the queue was created successfully.
* \return PAL_ERR_NO_MEMORY when there is no memory resource available to create an queue object.
*/
palStatus_t pal_StaticQueueCreate(const unsigned long queueLen, const unsigned long itemSize, unsigned char *buffer, palQueueID_t* queueID);

/*! \brief Check if the queue is empty OR not.
*
* @param[in] queueID   The queue ID for which the status is required.
* @param[out] isEmpty  Pointer to the flag which will hold the Queue status when returning.
* \return PAL_SUCCESS when the successfully get the status of the queue.
* \return PAL_ERR_GENERIC_FAILURE if queue ID is invalid OR other failures.
*/
palStatus_t pal_IsQueueEmpty(palQueueID_t queueID, bool *isEmpty);

/*! \brief Check if the queue is full OR not.
*
* @param[in] queueID   The queue ID for which the status is required.
* @param[out] isFull   Pointer to the flag which will hold the Queue status when returning.
* \return PAL_SUCCESS when the successfully get the status of the queue.
* \return PAL_ERR_GENERIC_FAILURE if queue ID is invalid OR other failures.
*/
palStatus_t pal_IsQueueFull(palQueueID_t queueID, bool *isFull);

/*! \brief Get the number of queued items in the queue .
*
* @param[in] queueID    The queue ID for which the item count is required.
* @param[out] itemCount Pointer to hold the Queued item count when returning.
* \return PAL_SUCCESS when the successfully get the queued item count.
* \return PAL_ERR_GENERIC_FAILURE if queue ID is invalid OR other failures.
*/
palStatus_t pal_GetQueueItems(palQueueID_t queueID, uint32_t *itemCount);
